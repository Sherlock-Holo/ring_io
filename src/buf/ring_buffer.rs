...
impl InnerBufRing {
    ...
    fn register(&self, submitter: &Submitter) -> io::Result<()> {
        let bgid = self.bgid;
    ...
    fn get_buf(&self, buf_ring: FixedSizeBufRing, res: u32, flags: u32) -> GBuf {
    ...
    fn buf_ring_push(&self, bid: Bid) {
        ...
        let old_tail = self.local_tail.fetch_add(1, Ordering::AcqRel);
    ...
    fn buf_ring_sync(&self) {
        unsafe {
            (*self.shared_tail).store(self.local_tail.load(Ordering::Acquire), Ordering::Release);
        }
    }
}
